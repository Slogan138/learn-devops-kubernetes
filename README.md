# Learn DevOps: Kubernetes

## What is it?

Udemy 에서 Learn DevOps: Kubernetes 강의를 수강하며 정리한 내용들입니다.

## Contents

### Section 2

- [Introduction](section02/introduction.md)
- [Containers](section02/containers.md)
- [Kubernetes Setup](section02/kubernetes_setup.md)
- [Minikube Setup](section02/minikube_setup.md)
- [Minikube vs. Docker Client vs. kops vs. kubeadm](section02/minikube_dockerclient_kops_kubeadm.md)
- [Kops](section02/kops.md)
- [Building Containers](section02/buliding_containers.md)
- [Docker Image Registry](section02/docker_image_registry.md)
- [Running First App in Kubernetes](section02/running_first_app.md)
- [Setting Load Balancer](section02/setting_load_balancer.md)

### Section 3

- [Node Architecture](section03/node_architecture.md)
- [Scailing Pods](section03/scailing_pods.md)
- [Deployments](section03/deployments.md)
- [Services](section03/services.md)
- [Labels](section03/labels.md)
- [Health Checks](section03/health_checks.md)
- [Readiness Probes](section03/readiness_probe.md)
- [Pod State](section03/readiness_probe.md)
- [Pod Lifecycle](section03/pod_lifecycle.md)
- [Secret](section03/secret.md)